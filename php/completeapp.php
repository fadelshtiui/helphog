...
if (isset($_SESSION['firstname']) && isset($_SESSION['lastname']) && isset($_SESSION['email']) && isset($_SESSION['password']) && isset($_SESSION['zip']) && isset($_SESSION['phone']) && isset($_SESSION['radius']) && isset($_SESSION['workfield']) && isset($_SESSION['experience']) && isset($_SESSION['stripe']) && isset($_SESSION['tz'])) { // regular apply
    ...
    $sql = "INSERT INTO login (firstname, lastname, email, password, phone, type, verified, zip, work_zip, work_phone, work_email, stripe_acc, verify_key, timezone) VALUES (:firstname, :lastname, :email, :password, :phone, :type, :verified, :zip, :work_zip, :work_phone, :work_email, :stripe_acc, :verify_key, :timezone);";
    $stmt = $db->prepare($sql);
    ...
    send_email("admin@helphog.com", "admin@helphog.com", "New Applicant", $firstname . "\n\n" . $email . "\n\n" . $workfield . "\n\n" . $experience . "\n\n" . $phone);
