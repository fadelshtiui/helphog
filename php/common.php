...
function get_user_info($session) {
	if ($session != '') {
		...
		if (hash_equals($result['session'], $session)) {
			...
			$result['match_session'] = $result['ios_provider_session'];
...
function send_claimed_task_ios($email, $title, $body, $color, $service, $local_time, $duration, $location, $price, $message, $ordernumber, $customer_email, $customer_phone)
{
    include 'constants.php';
	$db = establish_database();
...
function start_stop_order($order)
{
	...
	$service = '';
	$customer_email='';
	$stmnt = $db->prepare("SELECT status, schedule, service, customer_email FROM {$DB_PREFIX}orders WHERE order_number = ?;");
	$stmnt->execute(array($order));
	foreach ($stmnt->fetchAll() as $row) {
		$status = $row['status'];
		$schedule = $row['schedule'];
...
function claim_order($email, $order_number, $accept_key, $mobile, $apple)
{
	...
	if ($found && !$cancelled) {
		...
		$schedule = "";
		$customer_email = "";
		$stmnt = $db->prepare("SELECT people, schedule, service, customer_email FROM {$DB_PREFIX}orders WHERE order_number = ?;");
		$stmnt->execute(array($order_number));
		foreach ($stmnt->fetchAll() as $row) {
			$people = $row['people'];
			$service = $row['service'];
