    ...
    if (check_session($post_session)) {
        ...
        foreach($stmnt->fetchAll() as $row) {
            ...
            if ($row['status'] != 'st') {
                ...
                $entry->amount = round($earnings, 2) + $row["expenditure"];
            ...
            $mintotal = (strtotime($row['schedule']) - strtotime($now)) / 60;
            
            if ($mintotal < 1440) {
                $entry->warning = 'Warning: since you are canceling within 24 hours of the start time, you will be charged. ';
            }
